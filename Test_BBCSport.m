...
while(Isconverg == 0)
    ...
    % update E{k}
    F = [];
    F = [X{1}-X{1}*Z{1}+Y1{1}/mu;X{2}-X{2}*Z{2}+Y1{2}/mu];
    F = [F;X{1}-P{1}*H+Y2{1}/mu;X{2}-P{2}*H+Y2{2}/mu];
    F = [F;H-H*Z{K+1}+Y3/mu];
    [Econcat] = solve_l1l2(F,lambda/mu);

    E1{1} = Econcat(1:size(X{1},1),:);
    E1{2} = Econcat(size(X{1},1)+1:size(X{1},1)+size(X{2},1),:);
    tmp_index_E1 = size(X{1},1)+size(X{2},1);
    ...
    tmp_index_E2 = tmp_index_E1 + size(X{1},1)+size(X{2},1);
